...
// HTTPDigest defines a structrue which makes HTTP calls
//   with digest authentication (fill in URL, user, password and make a call)
...
// HTTPDigestPost sends JSON data via HTTP where digest authentication is used
//   the method requires the url, username and password defined in the HTTPDigest struct
//   the JSON data which are sent to the URL is represented with io.Reader
func (d *HTTPDigest) HTTPDigestPost(httpJSONData io.Reader) (*http.Response, error) {
	httpClient := &http.Client{Timeout: time.Second * 10}
	req, err := http.NewRequest("POST", d.URL, httpJSONData)
	if err != nil {
		return nil, fmt.Errorf("Fail to create HTTP request for hostname %s", d.URL)
	}
	req.Header.Set("Content-Type", "application/json")

	digestAuth := &DigestHeaders{}
	digestAuth, err = digestAuth.Auth(d.Username, d.Password, d.URL)
	if err != nil {
		return nil, fmt.Errorf("Fail to authenticate to %s for http WildFly management for username %s, operation %v, error %v",
			d.URL, d.Username, httpJSONData, err)
	}
