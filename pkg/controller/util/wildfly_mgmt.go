...
func GetTransactionRecoveryPort(pod *corev1.Pod, jbossHome string) (int32, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Error on management operation to read socket binding with command %v, error: %v",
			MgmtOpSocketBindingRead, err)
	}
	if !IsMgmtOutcomeSuccesful(jsonResult) {
		return 0, fmt.Errorf("Cannot read information on socket binding. The response on command '%v' was %v",
	...
	if err != nil {
		return 0, fmt.Errorf("Cannot read port offset with the socket binding read command '%v' with response %v. "+
	...
	if err != nil {
		return 0, fmt.Errorf("Cannot read txn recovery port with the socket binding read command '%v' with response %v. "+
