...
const (
	...
	statefulSetAnnotationForTxnSecretName       = "txn-recovery-secret-uid"         // used as key for storing txn recovery secret in statefulset annotations
)
...
func (r *ReconcileWildFlyServer) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// Verify that StatefulSet is consistent to the existing secret
	foundStatefulSetTxnSecretUID := foundStatefulSet.ObjectMeta.Annotations[statefulSetAnnotationForTxnSecretName]
	foundSecretUID := fmt.Sprintf("%v", foundTxnRecoverySecret.UID)
	reqLogger.Info("UIDs found: ", "statefulset uid marker", foundStatefulSetTxnSecretUID, "secret uid", foundSecretUID)
	if foundStatefulSetTxnSecretUID != foundSecretUID {
		// secret used in StatefulSet does not match the transaction recovery secret available, need to recreate the StatefulSet
		err = r.client.Delete(context.TODO(), foundStatefulSet)
		if err != nil {
			reqLogger.Error(err, "Cannot delete stateful set for re-matching with the available transaction recovery secret")
			return reconcile.Result{}, err
		}
		// StatefulSet deleted successfully - return and requeue to create a new one
		return reconcile.Result{Requeue: true}, nil
	}
	...
	for scaleDownIndex := 1; scaleDownIndex <= numberOfPodsToScaleDown; scaleDownIndex++ {
		...
		if wildflyServerSpecPodStatus == nil {
			scaleDownErrors = append(scaleDownErrors,
				fmt.Errorf("Cannot find pod name '%v' in the list of the active pods for the WildFly controller with data: %v", scaleDownPodName, wildflyServer))
			break
		}
	...
	statefulSet := &appsv1.StatefulSet{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				statefulSetAnnotationForTxnSecretName: fmt.Sprintf("%v", txnRecoverySecret.ObjectMeta.UID),
			},
		},
